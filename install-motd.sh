...
### Фикс (примечательно для Ubuntu), если:
# Некорректная работа IPv6 на VPS
# VPS без глобального IPv6
# mirror-сервера Ubuntu нестабильны по IPv6
...
[ "$SHOW_FIREWALL" = true ] && {
  ...
  if command -v "$UFW_BIN" &>/dev/null; then
    STATUS=$($UFW_BIN status | head -1 | awk '{print $2}')
    if [ "$STATUS" = "active" ]; then
      printf "${COLOR_LABEL}%-22s${COLOR_GREEN}%s${RESET}\n" "UFW Status:" "$STATUS"
      if [ "$SHOW_FIREWALL_RULES" = true ]; then
        mapfile -t RULES < <($UFW_BIN status | awk '/ALLOW/ {
  port=$1
  from=""
  for (i=3; i<=NF; i++) {
  ...
  print port "|" from
        ...
        if [ ${#SORTED_KEYS[@]} -gt 0 ]; then
          ...
          for ENTRY in "${SORTED_KEYS[@]}"; do
            FROM=${ENTRY#*|}
            PORTS=${GROUPED_RULES[$FROM]}
            PORTS=${PORTS%, }  # удалить последнюю запятую
...
"SHOW_FIREWALL_RULES" "Показать правила UFW" $(grep -q 'SHOW_FIREWALL_RULES=true' "$CONFIG" && echo ON || echo OFF) \
...
echo "[+] Creating 'rw-motd' command..."
cat << 'EOF' > /usr/local/bin/rw-motd
#!/bin/bash
/etc/update-motd.d/00-remnawave
EOF
chmod +x /usr/local/bin/rw-motd
...
### Фикс (при установке новой версии после старых - не отображался MOTD)
sed -i 's|^#\s*\(session\s\+optional\s\+pam_motd\.so\s\+motd=/run/motd\.dynamic\)|\1|' /etc/pam.d/sshd
sed -i 's|^#\s*\(session\s\+optional\s\+pam_motd\.so\s\+noupdate\)|\1|' /etc/pam.d/sshd

### Оставляем разрешение только кастомному MOTD
chmod -x /etc/update-motd.d/*
chmod +x /etc/update-motd.d/00-remnawave

### Отключаем стандартный /etc/motd (в Debian используется как статический MOTD)
rm -f /etc/motd
